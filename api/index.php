...
function getEffectiveRoomGender($pdo, $roomId) {
    $stmt = $pdo->prepare("
        SELECT r.gender as room_gender, f.gender as floor_gender, b.gender as building_gender
        FROM rooms r
    ...
    if ($row['room_gender'] !== 'DEFAULT') return $row['room_gender'];
    if ($row['floor_gender'] !== 'DEFAULT') return $row['floor_gender'];
    return $row['building_gender'];
}
...
try {
    ...
    if ($uri === 'book') {
        ...
        // ─── Проверка соответствия пола участника полу комнаты ───────────────
        $effectiveRoomGender = getEffectiveRoomGender($pdo, $roomId);
        if ($effectiveRoomGender === 'M' || $effectiveRoomGender === 'F') {
            $userLastName = $user['last_name'] ?? $data['last_name'] ?? '';
            $detectedUserGender = detectGenderByLastName($userLastName);

            if ($detectedUserGender !== null && $detectedUserGender !== $effectiveRoomGender) {
                $roomTypeLabel = ($effectiveRoomGender === 'M' ? 'мужской' : 'женской');
                jsonError("Вы не можете заселиться в эту комнату. Комната является {$roomTypeLabel}, а ваш определенный пол не совпадает с ней.", 400);
            }
        }
    ...
    if ($uri === 'auto-book') {
        ...
        // Проверяем соответствие пола по фамилии если определен
        $userLastName = $user['last_name'] ?? $data['last_name'] ?? '';
        $detectedUserGender = detectGenderByLastName($userLastName);
        if ($detectedUserGender !== null && $detectedUserGender !== $gender) {
            jsonError("Определенный пол участника по фамилии (" . ($detectedUserGender === 'M' ? 'мужской' : 'женский') . ") не совпадает с выбранным полом ({$gender}).", 400);
        }
    ...
    if (strpos($uri, 'admin/') === 0) {
        ...
        if ($uri === 'admin/users') {
            ...
            if ($method === 'POST') {
                ...
                if ($id > 0) {
                    $isCurator = in_array(strtolower(trim($curatorUser['role'])), ['curator', 'moderator'], true);
                    ...
                    if ($isCurator) {
                        if ($targetUser && strtolower(trim($targetUser['role'])) === 'admin') {
                            jsonError('Куратор не может редактировать администратора', 403);
                        }
                        if ($role === 'admin') {
                            jsonError('Куратор не может назначать роль администратора', 403);
                        }
                        if (!checkCuratorAccessToUser($pdo, $curatorUser['id'], $id) && $curatorUser['id'] != $id) {
...
<dyad-write path="src/pages/admin/AdminHomePage.tsx" description="Синхронизация глобального уведомления из БД с админкой">
...
export const AdminHomePage: React.FC = () => {
  ...
  // Загружаем текущее глобальное уведомление (передаем admin = true)
  const loadNotification = async () => {
    try {
      const data = await getGlobalNotification(true);
