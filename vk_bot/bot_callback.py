...
def format_task_message(task, report=None, task_status=None):
    ...
    msg += "\n📎 Для подтверждения выполнения прикрепите фото или ссылку. Если ты снял видео - прикрепи ссылку на него. Видео, прикрепленные к сообщению не будут распознаны."
    return msg
