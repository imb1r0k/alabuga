...
def create_report(user_id, task_id, submission_text, has_attachments=False):
    ...
    try:
        with conn.cursor() as cursor:
            # Проверяем, есть ли уже одобренный отчет для этого задания
            cursor.execute("""
                SELECT id FROM vk_bot_reports
                WHERE user_id = %s AND task_id = %s AND status = 'approved'
                LIMIT 1
            ...
            # Проверяем, есть ли уже ожидающий отчет
            cursor.execute("""
                SELECT id FROM vk_bot_reports
...
def update_report_status(report_id, status, reject_reason=''):
    ...
    try:
        with conn.cursor() as cursor:
            ...
            # Проверяем, не был ли уже одобрен этот отчет
            cursor.execute("""
            ...
            if status == 'approved':
                ...
                # Проверяем, все ли задания волны выполнены
                cursor.execute("""
                ...
                # Проверяем, не выдан ли уже билет за эту волну
                cursor.execute("""
                    SELECT id FROM vk_bot_tickets
