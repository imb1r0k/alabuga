...
def invalidate_user_task_cache(user_id, task_id):
    """
    Инвалидирует кэш статуса задания для пользователя.
    Используется после создания отчета, чтобы обновить отображение кнопки.
    """
    global _cache, _cache_time
    key = f'user_task_status_{user_id}_{task_id}'
    _cache.pop(key, None)
    _cache_time.pop(key, None)
    logger.info(f"🗑️ Инвалидирован кэш статуса для user_id={user_id}, task_id={task_id}")