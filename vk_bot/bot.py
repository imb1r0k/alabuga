        ...
        if event.type == VkEventType.MESSAGE_NEW and event.to_me:
            ...
            try:
                ...
                if not check_user_agreement(db_user['id']):
                    ...
                    # Важно: после отправки правил пропускаем дальнейшую обработку
                    continue
                ...
                if not check_user_agreement(db_user['id']):
                    ...
                    try:
                        ...
                        if os.path.exists(doc_path):
                            ...
                            vk.messages.send(
                                user_id=vk_id,
                                message="📋 Правила пребывания на Форуме:",
                                attachment=attachment,
                                random_id=0
                            )
                            logger.info(f"Файл с правилами отправлен пользователю {vk_id} при подтверждении")
                        else:
                            logger.warning(f"Файл с правилами не найден: {doc_path}")
                            # Отправляем текстовую версию правил
                            rules_text = (
                                "📋 Правила пребывания на Форуме\n\n"
                                "1. Участник обязан соблюдать правила пребывания.\n"
                                "2. Постоянно носить именной бейдж.\n"
