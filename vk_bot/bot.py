    ...
    try:
        ...
        if not os.path.exists(document_path):
            ...
            rules_text = (
                ...
                "✅ Нажмите кнопку 'Подтверждаю', если ознакомлены с правилами."
            )
            ...
            logger.info(f"Правила (текст) отправлены пользователю {user_id}")
            return
        
        # Получаем сервер для загрузки документа
        logger.info(f"Получение сервера для загрузки документа для пользователя {user_id}")
        upload_data = vk.docs.getMessagesUploadServer(type='doc', peer_id=user_id)
        upload_url = upload_data['upload_url']
        logger.info(f"URL для загрузки получен: {upload_url}")
        ...
        with open(document_path, 'rb') as file:
            ...
            file_param = result.get('file')
            
            if not file_param:
                logger.error("Не удалось загрузить файл, параметр 'file' отсутствует в ответе")
                # Отправляем текстовое сообщение с правилами
                rules_text = (
                    "📋 Правила пребывания на Форуме\n\n"
                    "1. Участник обязан соблюдать правила пребывания.\n"
        ...
        # Сохраняем документ
        logger.info(f"Сохранение документа для пользователя {user_id}")
        saved_doc = vk.docs.save(file=file_param, title="Правила пребывания на Форуме")
        
        if not saved_doc or len(saved_doc) == 0:
