...
export const AdminBuildingsPage: React.FC = () => {
  ...
  // Подтверждение удаления (корпуса, этажи, комнаты)
  ...
  const handleRunGeneration = () => {
    ...
    setHasUnsavedChanges(true);
    const successMsg = `Сгенерировано ${placedCount} комнат (${genFrom}–${genFrom + placedCount - 1})`;
      ...
      <div className="animate-fade-in" style={{ padding: '0 4px' }}>
        {buildingsLoading ? (
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '280px 1fr', gap: '20px' }}>
            ...
            <div className="admin-card" style={{ padding: '18px', height: 'fit-content' }}>
              ...
              <form onSubmit={handleAddBuilding} style={{ borderTop: '1px solid var(--border-color)', paddingTop: '16px' }}>
                ...
                <input
                  ...
                  style={{ width: '100%', padding: '8px 10px', marginBottom: '8px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-color)', fontSize: '13px' }}
                  required
                />
                <select
                  value={newBuildingGender}
                  onChange={(e) => setNewBuildingGender(e.target.value as any)}
                  style={{ width: '100%', padding: '8px 10px', marginBottom: '12px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-color)', fontSize: '13px' }}
                >
            ...
            {/* Основной блок: Редактор выбранного корпуса */}
            <div className="admin-card" style={{ padding: '24px' }}>
              {selectedBuilding ? (
                <div>
                  {/* Заголовок корпуса и глобальные кнопки действий */}
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '18px', flexWrap: 'wrap', gap: '12px', borderBottom: '1px solid var(--border-color)', paddingBottom: '16px' }}>
                        ...
                        {(() => {
                          ...
                          return (
                            <span style={{ fontSize: '12px', color: 'var(--text-secondary)', fontWeight: 500 }}>
                              Всего занято: <strong>{bStats.booked}</strong> из <strong>{bStats.total}</strong> мест
                            </span>
                          );
                        })()}
                      </div>
                    ...
                    <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                      ...
                      {/* Красивый Переключатель Режима Редактирования */}
                      <div 
                        onClick={() => {
                        ...
                        style={{
                          ...
                          padding: '6px 12px',
                          borderRadius: 'var(--radius-sm)',
                          backgroundColor: isEditLayout ? 'rgba(2, 132, 199, 0.1)' : 'var(--bg-app)',
                          border: `1px solid ${isEditLayout ? 'var(--accent-primary)' : 'var(--border-color)'}`,
                          cursor: 'pointer',
                          transition: 'var(--transition)'
                        }}
                      >
                        <span style={{ fontSize: '12px', fontWeight: 600, color: isEditLayout ? 'var(--accent-primary)' : 'var(--text-secondary)' }}>
                          {isEditLayout ? 'Редактирование' : 'Просмотр'}
                        </span>
                      ...
                      {isEditLayout && (
                        <button
                          ...
                          style={{
                            fontSize: '12px',
                            backgroundColor: genMode ? '#8b5cf6' : 'var(--bg-app)',
                            color: genMode ? '#ffffff' : 'var(--text-primary)',
                            border: '1px solid ' + (genMode ? '#8b5cf6' : 'var(--border-color)')
                          }}
                        >
                          <Zap size={15} /> {genMode ? 'Автогенерация' : 'Автогенерация'}
                        </button>
                      )}

                      <button onClick={openAddFloorModal} className="btn btn-secondary" style={{ fontSize: '12px' }}>
                        <Plus size={15} /> Добавить этаж
                      </button>
                    </div>
                  </div>

                  {/* Список кнопок этажей со статистикой x/y мест */}
                  ...
                  {selectedFloor ? (
                    <div>
                      {/* Панель опций этажа в режиме редактирования */}
                      {isEditLayout && (
                        <div style={{
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                            ...
                            <strong>Параметры этажа {selectedFloor.floor_number}:</strong>
                          </div>

                          <div>
                            <label style={{ marginRight: '6px', color: 'var(--text-secondary)' }}>Длина сетки:</label>
                            <input
                              type="number"
                              min={3}
                              max={20}
                              ...
                              style={{ width: '60px', padding: '4px 8px', borderRadius: '4px', border: '1px solid var(--border-color)' }}
                            />
                          </div>

                          <div>
                            <label style={{ marginRight: '6px', color: 'var(--text-secondary)' }}>Стартовый №:</label>
                            <input
                              type="number"
                              placeholder="Авто"
                              value={selectedFloor.start_room_number || ''}
                              onChange={(e) => handleUpdateFloorStartRoomNum(e.target.value ? Number(e.target.value) : null)}
                              style={{ width: '70px', padding: '4px 8px', borderRadius: '4px', border: '1px solid var(--border-color)' }}
                            />
                          </div>

                          <div>
                            <label style={{ marginRight: '6px', color: 'var(--text-secondary)' }}>Порядок:</label>
                            <select
                          ...
                          <div>
                            <label style={{ marginRight: '6px', color: 'var(--text-secondary)' }}>Пол:</label>
                            <select
                      ...
                      {isEditLayout && (
                        <div style={{ marginBottom: '18px' }}>
                          <div style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-secondary)', marginBottom: '8px' }}>
                            {genMode ? 'Маркеры генерации (перетащите на сетку или кликните):' : 'Заготовки элементов:'}
                          </div>
                              ...
                              return (
                                <div
                                  ...
                                  style={{
                                    ...
                                    border: `2px solid ${isSelected ? 'var(--accent-primary)' : tmpl.borderColor}`,
                      ...
                      {isEditLayout && genMode && (
                        <div style={{ backgroundColor: '#f3e8ff', border: '2px dashed #8b5cf6', padding: '16px', borderRadius: 'var(--radius-md)', marginBottom: '20px' }}>
                          <h4 style={{ margin: '0 0 10px 0', color: '#6b21a8', display: 'flex', alignItems: 'center', gap: '8px', fontSize: '14px' }}>
                            <Zap size={16} /> Генератор цепочки комнат по маркерам
                          </h4>
                          <div style={{ display: 'flex', gap: '16px', alignItems: 'center', flexWrap: 'wrap' }}>
                            <div>
                              <label style={{ fontSize: '12px', fontWeight: 600, marginRight: '6px' }}>От №:</label>
                              <input type="number" value={genFrom} onChange={(e) => setGenFrom(Number(e.target.value))} style={{ width: '70px', padding: '4px 8px', borderRadius: '4px', border: '1px solid #8b5cf6' }} />
                            </div>
                            <div>
                              <label style={{ fontSize: '12px', fontWeight: 600, marginRight: '6px' }}>До №:</label>
                              <input type="number" value={genTo} onChange={(e) => setGenTo(Number(e.target.value))} style={{ width: '70px', padding: '4px 8px', borderRadius: '4px', border: '1px solid #8b5cf6' }} />
                            </div>
                            <div>
                            ...
                            <button onClick={handleRunGeneration} className="btn btn-primary" style={{ fontSize: '12px', backgroundColor: '#7c3aed' }}>
                              <Play size={15} /> Сгенерировать
                            </button>
                          </div>
                          ...
                          {Array.from({ length: Number(selectedFloor.width) || 8 }).map((_, x) => {
                            ...
                            return (
                              <div
                                ...
                                style={{
                                  height: '95px',
                                  border: room ? `2px solid ${tmpl?.borderColor || 'var(--accent-primary)'}` : '2px dashed var(--border-color)',
                                  borderRadius: 'var(--radius-sm)',
                                  backgroundColor: room ? (tmpl?.bg || '#e0f2fe') : 'var(--bg-card)',
                                  color: tmpl?.textColor || 'var(--text-primary)',
                                  display: 'flex',
                              ...
                              >
                                  ...
                                  <div style={{ position: 'absolute', top: '3px', right: '4px', display: 'flex', alignItems: 'center', gap: '3px' }}>
                                    ...
                                    {isEditLayout && (
                                      <button
                                        ...
                                        style={{ border: 'none', background: 'rgba(239, 68, 68, 0.9)', color: '#fff', borderRadius: '50%', width: '15px', height: '15px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', padding: 0 }}
                                      >
                                        <X size={9} />
                                      </button>
                                    )}
                                ...
                                {room ? (
                                        ...
                                        {([
                                          ...
                                          <button
                                            ...
                                            style={{
                                              ...
                                              border: 'none',
                                              borderRadius: '3px',
                                              background: room.name.includes(dVal) ? 'var(--accent-primary)' : 'transparent',
                                              color: room.name.includes(dVal) ? '#fff' : 'var(--text-primary)',
                                              cursor: 'pointer',
                                              display: 'flex',
                                              alignItems: 'center'
                                            }}
                                  ...
                                  <div style={{ color: 'var(--text-muted)', fontSize: '11px', textAlign: 'center' }}>
                                    <span style={{ fontSize: '10px', display: 'block', fontWeight: 'bold', color: 'var(--text-muted)' }}>№ {calcRoomNum}</span>
                      ...
                      {selectedRoom && (
                        <div style={{
                          ...
                          {/* Список бронирований этой комнаты */}
                          <div style={{ marginBottom: '20px', backgroundColor: 'var(--bg-card)', padding: '14px', borderRadius: 'var(--radius-sm)', border: '1px solid var(--border-color)' }}>
                            <h4 style={{ margin: '0 0 10px 0', fontSize: '13px', color: 'var(--text-secondary)', display: 'flex', alignItems: 'center', gap: '8px' }}>
                            ...
                            {roomBookings.length > 0 ? (
                              ...
                              <p style={{ margin: 0, fontSize: '13px', color: 'var(--text-muted)' }}>На эту комнату нет активных бронирований.</p>
                            )}
                          </div>
                          ...
                          <form onSubmit={handleSaveRoomDetailsLocally} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px' }}>
                            ...
                            <div className="input-group" style={{ marginBottom: 0 }}>
                              ...
                              >
                                <option value="DEFAULT">От этажа/корпуса</option>
                                <option value="MIXED">Смешанный (С)</option>
                                <option value="M">Мужской (М)</option>
                                <option value="F">Женский (Ж)</option>
                              </select>
                              ...
                              {isEditLayout && (
                                <button
                                  ...
                                  onClick={() => {
                                    setDeleteConfirmTarget({
                                      type: 'room',
                                      x: Number(selectedRoom.x_pos),
                                      y: Number(selectedRoom.y_pos),
                                      name: selectedRoom.room_number || selectedRoom.name
                                    });
                                  }}
                                >
